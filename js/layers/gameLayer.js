...
define(
    ...
    function (cc, cp, MasterCircle, RedCircle, BlueCircle, i18n) {
        return cc.Layer.extend({
            ...
            ctor     :function () {
                ...
                this.space.addBody(this.myCircle.getBody());
                this.space.addShape(this.myCircle.shape);
            ...
            initCollision: function () {
                var me = this;
                this.myCircle.shape.setCollisionType(1);
                this.space.addCollisionHandler(1, 0, function (arbiter, space) {
                    var shapes = arbiter.getShapes();
                    var bodyA = shapes[0].getBody();
                    var bodyB = shapes[1].getBody();
                    var spriteA = bodyA.sprite;
                    var spriteB = bodyB.sprite;
                    var tmp;

                    if (spriteB.circleType === 'master') {
                        tmp = spriteA;
                        spriteA = spriteB;
                        spriteB = tmp;
                    }

                    if (spriteA.circleType === 'master') {
                        if (spriteB.circleType === 'red') {
                            spriteB.initPosition();
                            me.score ++;
                            me.scoreLabel.setString(i18n('Score: ') + me.score);

                            if (me.score % 2 === 0) {
                                setTimeout(function () {
                                    tmp = new BlueCircle();
                                    me.enemyCircle.push(tmp);
                                    me.space.addBody(tmp.getBody());
                                    me.space.addShape(tmp.shape);
                                    me.addChild(tmp, 3);
                                }, 100);
                            }
                        } else if (spriteB.circleType === 'enemy') {
                            me.gameOver();
                        }
            ...
            resetGame: function () {
                ...
                for (i = this.enemyCircle.length - 1, len = 15; i >= len; i --) {
                    tmp = this.enemyCircle.pop();
                    this.space.removeShape(tmp.shape);
                    this.space.removeBody(tmp.getBody());
